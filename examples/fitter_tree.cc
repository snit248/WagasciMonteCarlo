...
//  }
  ...
  while ((c = getopt(argc, argv, "i:o:s:e:")) != -1) {
    ...
    auto *out_seltree = new TTree("selectedEvents", "selectedEvents");
    auto *out_trutree = new TTree("trueEvents", "trueEvents");
    ...
    */
    ...
    int target_true;
    int cut_branch;
    float D1True; // True Muon momentum
    float D1Reco; // Reco Muon momentum
    float D2True; // True Muon angle
    float D2Reco; // Reco Muon angle
    float enu_true; // True neutrino energy
    float enu_reco; // Reco neutrino energy
    float q2_true; // True q2
    float q2_reco; //Reco q2
    float weight;
    float weight_true;
    float angle;
    float angle_r;

    float NU_E, NU_E_R; // True and Reco neutrino energy
    float mup, nue;
...
bool coincidence = false;
 ...
 for (int run_number = start_number; run_number <= end_number; run_number++) {
   ...
   input_file_name.Form("%s%04d.root", input_file_path.Data(), run_number);

   std::ifstream ifs(input_file_name.Data());
   if(!ifs.is_open())
     continue;
...
// A random number sampled from a gaussian is added to each  nu_energy value to create reconstructed energy
	...
	if(event->GetInteractionType() != 1 && event->GetInteractionType() != 2)
	  ...
	  coincidence = true;
	  reaction = event->GetInteractionType();
	  nutype = event->GetPrimaryParticlePdg();
	  topology = event->GetInteractionType()-1;
	  cut_branch = 1;
	  //weight = event->GetNormalization();
	  weight = 1.0;
   	  enu_true = event->GetPrimaryParticleEnergy().GetValue();
...
// B2VertexSummary::GetInteractionMaterial
    ...
    coincidence = true;
    target = vertex->GetInteractionMaterial();
	}
...
// A random number sampled from a gaussian is added to each  mu_momentum and mu_energy value to create reconstructed mu momentum and angle (D1 and D2)
	...
	D1True = track->GetInitialAbsoluteMomentum().GetValue();
// Reconstructed energy values smaller than 0 (not physical) are set to 0
	...
	angle = track->GetAngle().GetValue();
	angle_r = track->GetAngle().GetValue()+random_mu_ang;
	D2True = cos(track->GetAngle().GetValue());
        D2Reco = cos((track->GetAngle().GetValue())+random_mu_ang);
