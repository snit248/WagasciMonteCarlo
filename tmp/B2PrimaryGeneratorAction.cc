...
#include "B2NeutrinoGenerator.hh"

#define any boost::any_cast

B2PrimaryGeneratorAction::B2PrimaryGeneratorAction(GeneratorReader &generator,
                                                   B2RunAction *rac,
                                                   B2EventAction *evt,
						   B2DetectorConstruction *detcon,
    ...
    : particle_gun_(1), // num_particles = 1
      ...
      wagasci_target_separation_(any<bool>(settings.At(B2Settings::WAGASCITargetSeparation))),
  ...
  p_run_action_->CloneNEUTTree(generator_.get().GetGeneratorTree());
...
void B2PrimaryGeneratorAction::GenerateNeutrino(G4Event *g4_event) {
  ...
  const GeneratorEvent *p_neut_event = ReadGeneratorEvent();
  ...
  if( interaction_material_->GetName().contains("Water") == 1 ){
    primary_vertex.SetInteractionMaterial(0);
  }else if( interaction_material_->GetName().contains("Scintillator") == 1 ){
    primary_vertex.SetInteractionMaterial(1);
  }else if( interaction_material_->GetName().contains("Air") == 1 ){
    primary_vertex.SetInteractionMaterial(0);
  }else if( interaction_material_->GetName().contains("Iron") == 1 ){
    primary_vertex.SetInteractionMaterial(2);
  }else if( interaction_material_->GetName().contains("Concrete") == 1 ){
    primary_vertex.SetInteractionMaterial(3);
  }else if( interaction_material_->GetName().contains("SUS304") == 1 ){
    primary_vertex.SetInteractionMaterial(4);
  }else{
    primary_vertex.SetInteractionMaterial(5);
  }

  //std::cout << interaction_material_ << '\n';
  //std::cout << neutrino_interaction_material_ << '\n';
  ...
  if (p_neut_event->neutrino.primary_particle.particle_id !=
      generator_.get().ProductionMode2PdgId(p_neut_event->neutrino.production_mode)) {
    BOOST_LOG_TRIVIAL(debug) << "Neutrino flavor mismatch! "
                             ...
                             << B2Pdg::Stringify(generator_.get().ProductionMode2PdgId(p_neut_event->neutrino.production_mode));
  }
  ...
  for (int ipart = 0; ipart < p_neut_event->secondary.num_particles; ipart++) {
    ...
    if (p_neut_event->secondary.tracking_flag.at(ipart) == SecondaryParticles::TrackingFlag::kExitNucleus) {
      ...
      //BOOST_LOG_TRIVIAL(info) << SecondaryParticles::TrackingFlag::kExitNucleus << " " << p_neut_event->secondary.tracking_flag.at(ipart)
      //		      << " " << p_neut_event->secondary.particle_id[ipart];
...
const GeneratorEvent *B2PrimaryGeneratorAction::ReadGeneratorEvent() {
  const GeneratorEvent *p_neut_event;
  bool have_interaction = false;

  if(any<bool>(settings_.At(B2Settings::InitializeRandomSeedEvt)))
    {

      const auto seed = long( any<int>(settings_.At(B2Settings::FixedRandomSeed)) + p_run_action_->number_simulated_events );
      CLHEP::HepRandom::setTheSeed(seed, 4);

    }

  //std::cout << p_run_action_->number_simulated_events << '\n';

  // Read NEUT events until we find one that satisfies the user
  // settings and selection criteria
  while (!have_interaction) {
    if (generator_.get().ReadNextEvent() == B2InteractionMode::MODE_ERROR) {
    ...
    p_neut_event = &generator_.get().GetEvent();
    ...
    auto neutrino_flavor = generator_.get().ProductionMode2NeutrinoFlavor(p_neut_event->neutrino.production_mode);
	...
	interaction_vertex_.SetXYZ(x * cm, y * cm, 0.);
      ...
      //Function to separate target in WAGASCI interactions
      if(have_interaction){
	TVector3 temp_vertex_ = interaction_vertex_;
	temp_vertex_.SetX(temp_vertex_.X() + offset_x_);
	temp_vertex_.SetY(temp_vertex_.Y() + offset_y_);
	temp_vertex_.SetZ(temp_vertex_.Z() + offset_z_);
	
	G4VPhysicalVolume* world = p_detcon_->GetWorld();
	G4Navigator* g4Navigator = new G4Navigator();
	g4Navigator->SetWorldVolume( world );
	G4ThreeVector vertex(temp_vertex_.Px(), temp_vertex_.Py(), temp_vertex_.Pz());
	G4VPhysicalVolume* volume = g4Navigator->LocateGlobalPointAndSetup(vertex);
	auto material = volume->GetLogicalVolume()->GetMaterial();
	interaction_material_ = material;
	
	if( wagasci_target_separation_ && neutrino_interaction_material_ == B2Material::kWater ){
	  if( material->GetName().contains("Water") == 0 ){
	    have_interaction = false;
	  }
	}else if( wagasci_target_separation_ && neutrino_interaction_material_ == B2Material::kCarbon ){
	  if( material->GetName().contains("Scintillator") == 0 ){
	    have_interaction = false;
	  }
	}
      }
